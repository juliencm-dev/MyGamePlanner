  ...
  return (
    <div className="flex flex-col gap-2">
      {members.map(member => {
        ...
        return (
          <div key={member.id} className={`relative flex flex-col gap-4 border py-3 px-5 rounded-lg bg-gradient-to-t from-secondary/10 z-10`}>
            <div className={`flex items-center gap-5 ${toggle ? "cursor-pointer" : ""}`} onClick={() => handleMemberClick(member)}>
            ...
            {selectedMemberId === member.id && toggle && <UpdateMember member={member} setSelected={setSelectedMemberId} />}
            {selectedMemberId === member.id && toggle && <RemoveMemberButton groupMemberDto={member} setSelected={setSelectedMemberId} />}
          </div>
        );
      })}
    </div>
